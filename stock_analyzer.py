...
def main():
    ...
    # --- 1. SESSION STATE INICIALIZACE ---
    # Musíme si pamatovat stav menu I stav analýzy (protože rerun resetuje tlačítka)
    ...
    # Funkce pro otevření menu (volaná z hlavního okna)
    def _open_sidebar():
        st.session_state["sidebar_state"] = "expanded"
        # Nepoužíváme rerun tady, stačí nechat doběhnout script

    # --- 2. CSS A KONFIGURACE ---
    # initial_sidebar_state funguje jen při prvním loadu, ale necháme ho tu
    st.set_page_config(
    ...
    # AGRESIVNÍ CSS SKRÝVÁNÍ
    # Pokud je stav 'collapsed', vložíme CSS, které menu okamžitě zneviditelní
    sidebar_css = ""
    ...
    # Tlačítko MENU v hlavním okně (jen když je sidebar zavřený)
    if st.session_state["sidebar_state"] == "collapsed":
        st.button("☰ Změnit parametry", key="open_menu_btn", on_click=_open_sidebar)

    # Proměnná, která nahradí původní analyze_btn
    # Defaultně bereme hodnotu ze session state (aby přežila rerun)
    ...
    with st.sidebar:
        ...
        ticker_input = st.text_input(
            ...
            help="Zadej ticker (např. AAPL, MSFT)",
            max_chars=10,
        ).upper().strip()

        # --- HLAVNÍ ZMĚNA ZDE ---
        # Tlačítko, které spouští logiku a zavírá menu
        if st.button("🔍 Analyzovat", type="primary", use_container_width=True, key="trigger_analysis"):
            st.session_state["sidebar_state"] = "collapsed" # Zavřít menu
            st.session_state["analysis_active"] = True      # Aktivovat analýzu
            analyze_btn = True                              # Lokální update pro tento běh
            st.rerun()                                      # OKAMŽITÝ RESTART aplikace
